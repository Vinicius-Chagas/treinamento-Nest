...
export class AuthService {
  ...
  async login(dto: LoginDto) {
    ...
    const { token } = this.generateTokenByUserIdAndRole(user.id, user.role)

    if (!user.active) {
      throw new ForbiddenError('inativo')
    }
  ...
  private generateTokenByUserIdAndRole(id:number, role: Roles): { token: string } {
    const payload: PayloadType = {id, role}
